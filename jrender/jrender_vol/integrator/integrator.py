...
def integrator_opacity(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        raw: [num_rays, num_samples along ray, 4]. Prediction from model.
        z_vals: [num_rays, num_samples along ray]. Integration time.
    ...
    noise = 0.
    if raw_noise_std > 0.:
        noise = jt.init.gauss(raw.shape, raw.dtype) * raw_noise_std
    alpha = raw2alpha(raw[..., 0] + noise, dists)  # [N_rays, N_samples]
    alpha_num = jt.sum((alpha > 0.5).float().detach(), dim=-1, keepdims=True)
    # print (alpha.shape, alpha_num.shape)
    alpha = alpha / (alpha_num + 1e-6)
    weights = alpha * jt.cumprod(jt.concat([jt.ones((alpha.shape[0], 1)), 1.-alpha + 1e-10], -1), -1)[:, :-1]
    # rgb_map = jt.sum(weights.unsqueeze(-1) * rgb, -2)  # [N_rays, 3]
    alpha_sum = jt.sum(alpha, dim=-1, keepdims=True)
    # rgb_map = alpha_sum
    rgb_map = jt.repeat(alpha_sum, (1, 3))
