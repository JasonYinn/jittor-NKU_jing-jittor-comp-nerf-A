...
# Model
class ParallelNeRF(nn.Module):
    def __init__(self, number = 8, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False, W_view_linear=1):

        super(ParallelNeRF, self).__init__()
        
        w = int(W // number)
        self.num_nerf = number - 1
        self.nerfs = nn.ModuleList([NeRF(D, w, input_ch, input_ch_views, output_ch, skips, use_viewdirs, W_view_linear) for _ in range(number - 1)])
        self.to_weights = nn.Sequential(NeRF(D, w, input_ch, input_ch_views, number, skips, False, W_view_linear), nn.Softmax(dim=-1))

    def execute(self, x):
        raw_list = [self.nerfs[idx](x) for idx in range(self.num_nerf)]
        raw = jt.stack(raw_list, dim=-2)
        weights = self.to_weights(x)

        outputs = jt.sum(raw * weights.unsqueeze(-1), dim=-2)
        
        return outputs

# Model
class OpacityNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False, W_view_linear=1):
        """ 
        """
        super(OpacityNeRF, self).__init__()
        self.D = D
        self.W = W
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False, W_view_linear=1):
        ...
        self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W, W//2)] + [nn.Linear(W//2, W//2) for _ in range(W_view_linear - 1)])
...
# Model
class OffsetNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        """ 
        """
        super(OffsetNeRF, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
        ...
        if use_viewdirs:
            self.feature_linear = nn.Linear(W, W)
            self.offset_linear = nn.Linear(W//2, 3)
        else:
            self.output_linear = nn.Linear(W, output_ch)
...
# Model
class AttnNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        """ 
        """
        super(AttnNeRF, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
        ...
        attn_dim = W // 2
        self.attention = attention.MultiheadAttention(attn_dim, 4, dropout = 0.1, self_attention = True)

        self.init_layer = nn.Sequential(nn.Linear(input_ch, attn_dim), nn.ReLU())
        input_dim = attn_dim
        self.pts_linears = nn.ModuleList(
            [nn.Linear(input_dim, W)] + [nn.Linear(W, W) if i not in self.skips else nn.Linear(W + input_ch, W) for i in range(D-1)])
    ...
    def execute(self, x):
        ...
        h = self.init_layer(h)
        attn_inp = jt.unsqueeze(h, 0)
        attn = self.attention(attn_inp, need_weights = False)[0]
        attn = jt.squeeze(attn, 0)
        h = self.dropout(h) + attn
...
class DepthNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False):
        ...
        ### Implementation according to the official code release (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        self.views_linears = nn.ModuleList([nn.Linear(input_ch_views + W, W//2)] + [nn.Linear(W//2, W//2) for i in range(2)])
